data "template_file" "clixx_bootstrap" {
  template = file(format("%s/scripts/ecs_clixx_btstrap.tpl", path.module))
  vars={
    #GIT_REPO ="https://github.com/Teyojessam-Abam-14/CliXX_Retail_Repository.git"
    EFS_DNS = module.EFS-BASE[0].efs_dns
    DB_HOST = local.DB_HOST_CLIXX
    CLIXX_LB = module.ASG-LB-BASE[0].lb_dns
    CLUSTER_NAME = module.ECS-BASE[0].cluster_name
