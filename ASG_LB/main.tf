...
resource "aws_autoscaling_group" "terraform_asg"{
  ...
  tag {
    key                 = var.ASG_TAGS["server_key"]
    value               = var.ASG_TAGS["server_value"]
    propagate_at_launch = var.ASG_DETAILS["propagate_at_launch"]
  } 

  tag {
    key                 = var.ASG_TAGS["ecs_managed_key"]
    value               = var.ASG_TAGS["ecs_managed_value"]
    propagate_at_launch = var.ASG_DETAILS["propagate_at_launch"]
  }

  tag {
    key                 = var.ASG_TAGS["ecs_cluster_key"]
