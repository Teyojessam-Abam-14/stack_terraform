...
#ECS-optimized AMI (use this as a replacement for the above)
variable "ami" {
  default = "ami-02b4097aa70fb9ec4"
}

#Finding an ECR repo that contains a desired image 
data "aws_ecr_repository" "image" {
  name        = "clixx-repository"
  registry_id = "721636561061"
}
...
variable "clixx_name"{
  default = "ecs-clixx-lt-terraform"
}

#Name for Load Balancer
variable "lb_name"{
   default = "ecs-stack-lb-terraform"
}

#Name for Target Group
variable "tg_name"{
  default = "ECS-Stack-ClIXX-TG"
}

#Name for Autoscaling Group
variable "asg_name"{
  default = "ecs-clixx-asg-terraform"
}
...
variable "LT_DETAILS" {
  ...
  default = {
    ...
    instance_tag_resource_type = "instance"
    instance_tag_name = "ecs-instance"
    volume_type = "gp2"
    volume_size = 8
...
#Autoscaling group tags (for ECS container servers mainly)
variable "ASG_TAGS"{
  type=map(string)
  default={
    server_key="Name"
    server_value="Stack-ECS-Server-TF"
    ecs_managed_key="AmazonECSManaged"
    ecs_managed_value=true
    ecs_cluster_key="ECS_CLUSTER"
  }
}
...
variable "container_name" {
  default="clixx-cont-TF"
}

#Container image (localizing the data as a variable called "container_image")
locals {
  container_image="${data.aws_ecr_repository.image.repository_url}:latest"
}
...
variable "TASK_DEF_DETAILS" {
  ...
  default={
    ...
    task_execution_role_arn="arn:aws:iam::721636561061:role/ecsTaskExecutionRole"
    container_network_mode="awsvpc"
    protocol="tcp"
  } 
}

#ECS container memory and port mapping details 
